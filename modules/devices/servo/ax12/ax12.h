 ...
 *
 *  Revision : $Id: ax12.h,v 1.1.4.4 2009-04-07 20:00:57 zer0 Exp $
 *
 */
 ...
 *  on success. */
uint8_t AX12_send(AX12 *ax12, AX12_Packet *packet);

/* @brief Receive a formated AX12 status packet. Return 0 on
 *  on success. */
uint8_t AX12_recv(AX12 *ax12, AX12_Packet *packet);
...
uint8_t AX12_write_byte(AX12 *ax12, uint8_t id, AX12_ADDRESS address,
			uint8_t data);
 ...
 * address+1 : data high
 */
uint8_t AX12_write_int(AX12 *ax12, uint8_t id, AX12_ADDRESS address,
		       uint16_t data);

/** @brief Read byte from AX-12 memory 
 * @return Error code from AX-12 (0 means okay) */
uint8_t AX12_read_byte(AX12 *ax12, uint8_t id, AX12_ADDRESS address,
		       uint8_t *val);

/** @brief Write integer (2 bytes) from AX-12 memory
 * @return Error code from AX-12 (0 means okay) */
uint8_t AX12_read_int(AX12 *ax12, uint8_t id, AX12_ADDRESS address,
		      uint16_t *val);

/*___________ High level layer _________*/

/** @brief Set AX12 position */
uint8_t AX12_set_position(AX12 *ax12,uint8_t id, uint16_t position);

/** @brief Set AX12 position and moving speed */
uint8_t AX12_set_position2(AX12 *ax12, uint8_t id, uint16_t position,
			   uint16_t speed);

/** @brief Set AX12 position, moving speed and torque */
uint8_t AX12_set_position3(AX12 *ax12, uint8_t id, uint16_t position,
			   uint16_t speed, uint16_t torque);

/** @brief Read AX12 position */
uint8_t AX12_get_position(AX12 *ax12, uint8_t id, uint16_t *pos);

/** @brief Read AX12 speed */
uint8_t AX12_get_speed(AX12 *ax12, uint8_t id, uint16_t *speed);

/** @brief Read AX12 load */
uint8_t AX12_get_load(AX12 *ax12, uint8_t id, uint16_t *load);


/** @brief Ping an AX12 and return error register */
uint8_t AX12_ping(AX12 *ax12, uint8_t id);

/** @brief Reset AX12 back to factory settings */
uint8_t AX12_reset(AX12 *ax12, uint8_t id);
